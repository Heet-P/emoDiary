...
async def list_journal_entries(
    ...
    entries = await journal_service.get_entries(user_id, limit=limit, offset=offset, db=db)
    return entries
...
async def get_journal_count(
    ...
    count = await journal_service.get_entry_count(user_id, db=db)
    return {"count": count}
...
async def get_journal_entry(
    ...
    entry = await journal_service.get_entry(user_id, entry_id, db=db)
    if not entry:
...
async def delete_journal_entry(
    ...
    deleted = await journal_service.delete_entry(user_id, entry_id, db=db)
    if not deleted:
        raise HTTPException(status_code=404, detail="Journal entry not found")
    return None
