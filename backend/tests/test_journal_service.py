...
class TestCreateEntry:
    ...
    async def test_creates_entry_with_word_count(self):
        ...
        with patch("app.services.journal_service._generate_journal_analysis", new_callable=AsyncMock, return_value=mock_analysis):
            from app.services.journal_service import create_entry
            result = await create_entry("u1", "My day", "Hello world today", db=supabase, emotion_tag="Happy")
    ...
    async def test_raises_on_supabase_failure(self):
        ...
        with patch("app.services.journal_service._generate_journal_analysis", new_callable=AsyncMock, return_value=mock_analysis):
            ...
            with pytest.raises(Exception, match="Failed to create journal entry"):
                await create_entry("u1", "Title", "Content", db=supabase)
...
class TestGetEntries:
    ...
    async def test_returns_list(self):
        ...
        result = await get_entries("u1", db=supabase)
...
class TestDeleteEntry:
    ...
    async def test_returns_true_on_success(self):
        ...
        result = await delete_entry("u1", "e1", db=supabase)
    ...
    async def test_returns_false_when_not_found(self):
        ...
        from app.services.journal_service import delete_entry
        result = await delete_entry("u1", "00000000-0000-0000-0000-000000000000", db=supabase)
