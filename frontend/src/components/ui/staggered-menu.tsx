...
export const StaggeredMenu = React.forwardRef<StaggeredMenuHandle, StaggeredMenuProps>(({
    ...
    useLayoutEffect(() => {
        const ctx = gsap.context(() => {
            ...
            // Only init button refs when header is rendered
    ...
    const buildOpenTimeline = useCallback(() => {
        ...
        layers.forEach((ls, i) => {
            const start = Number(gsap.getProperty(ls, "xPercent"));
            tl.fromTo(
                ls,
                { xPercent: start },
    ...
    const playClose = useCallback(() => {
        ...
        closeTweenRef.current = gsap.to(all, {
            ...
            onComplete: () => {
                ...
                if (numEls.length)
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    gsap.set(numEls, { ["--sm-num-opacity" as any]: 0 });
                ...
                >
                    <div className="sm-panel-inner flex-1 flex flex-col gap-5">
                        ...
                        {displaySocials && socialItems && socialItems.length > 0 && (
                            ...
                            >
                                <h3 className="sm-socials-title m-0 text-base font-medium text-[var(--sm-accent,#0da2e7)]">
                                    Socials
                                </h3>
                                <ul
...
.sm-scope .sm-panel-item:hover { color: var(--sm-accent, #0da2e7); }
.sm-scope .sm-panel-list[data-numbering] { counter-reset: smItem; }
.sm-scope .sm-panel-list[data-numbering] .sm-panel-item::after { counter-increment: smItem; content: counter(smItem, decimal-leading-zero); position: absolute; top: 0.1em; right: 3.2em; font-size: 18px; font-weight: 400; color: var(--sm-accent, #0da2e7); letter-spacing: 0; pointer-events: none; user-select: none; opacity: var(--sm-num-opacity, 0); }
.sm-scope .sm-socials-list:hover .sm-socials-link:not(:hover) { opacity: 0.35; }
.sm-scope .sm-socials-link:hover { color: var(--sm-accent, #0da2e7); }
